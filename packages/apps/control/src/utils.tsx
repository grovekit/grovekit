...
import { Datatype, PropertyFormat, RawValue } from '@grovekit/homie-core';
...
const package_info: { name: string; version: string; } = JSON.parse(readFileSync(path.resolve(package_path, 'package.json'), 'utf8'));

export const PKG_NAME = package_info.name;
export const PKG_VERS = package_info.version;


export const prettyRawNumber = (value: number, datatype: 'integer' | 'float'): string => {
  return value.toFixed(3);
};

export const prettyRawValue = (value: any, datatype: Datatype): string => {
  switch (datatype) {
    case 'integer':
      return prettyRawNumber(parseInt(value), datatype);
    case 'float':
      return prettyRawNumber(parseFloat(value), datatype);
    default:
      return String(value);
  }
};
