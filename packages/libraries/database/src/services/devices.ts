        ...
        if (node_info.properties) {
          for (const [prop_h_id, prop_info] of Object.entries(node_info.properties)) {
            ...
            if (!prop) {
              ...
              updateFeedById(trx, prop.set_fid, { datatype: format.datatype });
              updateFeedById(trx, prop.value_fid, { datatype: format.datatype });
              updateFeedById(trx, prop.target_fid, { datatype: format.datatype });
            }
          }
        }
      }
